...
const cfg = (): SolarLoopConfig => ({
  ...
  deadbandA: 1, rampA: 3, minOnMs: 1000, minOffMs: 1000, marginW: 0, belowMin: 'pause',
});
...
test('below-min pauses at 0A then resumes (belowMin=pause)', () => {
  const l = new SolarLoop(cfg());
  l.evaluate({ gridSignedW: -2300, chargerPowerW: 0, now: T });
...
test('below-min stops the transaction (belowMin=stop)', () => {
  const c = cfg(); c.belowMin = 'stop';
  const l = new SolarLoop(c);
  l.evaluate({ gridSignedW: -2300, chargerPowerW: 0, now: T });
  const r = l.evaluate({ gridSignedW: 500, chargerPowerW: 0, now: T + 1500 });
